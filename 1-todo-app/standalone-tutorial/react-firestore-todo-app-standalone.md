# 🚀 React Todo App with Firebase Firestore
...
npm -v
```
If you get an error that says `command not found`, you need to install Node, download it here: [https://nodejs.org](https://nodejs.org) and choose the macOS installer.
...
4. Select region → Enable (Choose the closest if given the option)
...
Go back to the VS Code, open up a second terminal window (Terminal → New Terminal) and run:

```bash
npm install firebase
```

This created a `node_modules` folder with the Firebase library and added it to your `package.json` dependencies.

You also now have a second terminal window to run commands in, so you can keep your development server running in the first one.
...
**Goal:** Remove a todo from Firestore and the UI.

**Hints:**
- Import `deleteDoc` and `doc` from `firebase/firestore`.
- Pass each todo’s `id` into a delete handler.
- Use `await deleteDoc(doc(db, "todos", id))`.
- Add a **Delete** button next to each todo in `.map()`.
...
**Goal:** Toggle a `completed` field (`true/false`) for each todo.

**Hints:**
- When adding a todo, include `completed: false`.
- Import `updateDoc` and `doc`.
- Build a toggle function:
    - read current `completed`
    - call `updateDoc(doc(db, "todos", id), { completed: !completed })`
- In UI, show completed items differently (for example, line-through style).

Docs:
- [Update a document (Firestore)](https://firebase.google.com/docs/firestore/manage-data/add-data#update-data)
- [Conditional rendering (React)](https://react.dev/learn/conditional-rendering)

---

### 3) Add Timestamps
**Goal:** Store when each todo was created.

**Hints:**
- Import `serverTimestamp` from `firebase/firestore`.
- In `addDoc`, include `createdAt: serverTimestamp()`.
- Optionally sort by time using a query (`orderBy("createdAt", "desc")`).
...
### 4) Auto-Update with Realtime Listeners
**Goal:** Update the todo list instantly when Firestore changes.

**Hints:**
- Replace `getDocs(...)` in `useEffect` with `onSnapshot(...)`.
- Build a query for your `todos` collection (optional: include `orderBy`).
- In the snapshot callback, map docs to state.
...
💡 **Tip:** Try implementing one challenge at a time, then test before moving to the next.

---

## 📚 Helpful Official Tutorials
...
🎯 **You now built a full‑stack React + Firebase app!**
